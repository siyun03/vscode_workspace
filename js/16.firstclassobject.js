...
const add = function(a, b){
    ...
    // 유사배열객체 : 객체인데 배열처럼 사용할 수 있는 객체 (length 프라퍼티가 있음)
    console.log(arguments); // [Arguments] { '0': 1, '1': 2 }
    console.log(arguments.length); // 2
    // caller는 비표준 프라퍼티이므로 사용을 권장하지 않음
    console.log(add.caller); // [Function (anonymous)] // 익명function
...
// Object.getOwnPropertyDescriptors : 프로퍼티 디스크립터들을 조회
// Object.getOwnPropertyDescriptor  : 프로퍼티 디스크립터를 조회
// add는 함수 = 일급객체 => 함수 일급객체의 프라퍼티의 디스크립터를 조회
console.log(Object.getOwnPropertyDescriptors(add));
/*
프라퍼티는 데이터프라퍼티와 접근자프라퍼티로 나뉨
프라퍼티 수 : 5
프라퍼티마다 디스크립터 객체를 가짐
디스크립터 객체는 아래와 같은 프라퍼티를 갖는다
value : 프라퍼티의 값 (데이터프라퍼티는 value를 가짐)
writable : 프라퍼티의 값 수정 여부
enumerable : 프라퍼티 열거 여부
...
// 모든 객체는 __proto__라는 프라퍼티를 가짐
// __proto__는 prototype의 참조를 저장하고 있는 프라퍼티
console.log(person.__proto__ === Object.prototype); // true
// hasOwnProperty : 직접 생성한 프라퍼티인지 여부를 반환
console.log(person.hasOwnProperty('name')); // true
console.log(person.hasOwnProperty('age')); // true
console.log(person.hasOwnProperty('__proto__')); // false // 존재하긴 하나 직접 만들지 않아 F

const obj = {};
console.log(obj.__proto__ === Object.prototype); // true
// __proto__ : 모든 객체가 가지고 있으며, 상위타입에 있는 프로토를 나타냄

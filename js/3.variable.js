...
2.자바 스크립트 변수의 선언, 초기화, 할당
1) 선언 (declearation) : J/S에서 선언은 타입을 지정하지 않음. 단지 변수임만 선언
 ...
 * 만약에 자바스크립트 변수에 값이 변경되면 갑의 타입에 따라서 변수의 타입이 변경됨
...
// 선언 : 변수명만 알 수 있음, 초기화 하지 않으면 undefind로 초기화됨
let l2; // undefind로 자동 초기화 됨, 타입은 undefind < 타입이자 값 (초기화되지 않은 상태) , null은 null로 초기화된 값임
console.log(l2);

// 할당 : 자바스크립트는 변수에 값을 초기화하거나 할당할 때 변수의 타입이 정해진다 (동적 타이핑 언어)
l2 = 3; // l2는 number 타입
console.log(l2);
console.log(typeof l2); // typeof 는 타입을 보여줌 > number
l2 = 'hello';
console.log(l2);
console.log(typeof l2); // string

// const c2; // Missing initializer in const declaration / SyntaxError> 상수는 반드시 초기화 해야함

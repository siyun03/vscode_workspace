...
const prettyJon = JSON.stringify(person, null, 2);
console.log(typeof prettyJon, prettyJon);

// 객체를 JSON문자열로 변환 (replacer, 들여쓰기)
const filteredJson = JSON.stringify(
    // 변환할 객체
    person, 
    // replacer 함수 : 변환시에 필터링하는 함수, 원하는 프라퍼티만 변환하기 위해서 사용
    (key, value) => {
...
// open(HTTP요청 메소드, 요청URL)
// HTTP요청 메소드 : GET, POST, PUT, PATCH, DELETE
//                           GET : 요청헤더만 전송, 조회
...
// onreadyStatechange : readyState 프라퍼티의 값이 변경될 때마다 실행될 함수를 저장하는 프라퍼티
//                                  readyState가 0 > 1> 2> 3> 4 변경될 때마다 실행
xhr.onreadystatechange = () => {
    // 요청완료시에만 처리
    if (xhr.readyState !==4) return false;
    // 서버 응답이 정상완료되면 
    if (xhr.status === 200){
        // 서버에서 보낸 응답 body를 JSON객체로 변환해서 출력
        console.log(typeof xhr.response, xhr.response);
        // 응답body에 JSON문자열을 객체로 변환
